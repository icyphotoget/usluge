...
export default function OglasiClient() {
  ...
  const [type, setType] = useState<"" | "offer" | "request">((sp.get("type") as any) ?? "");
  const [catId, setCatId] = useState<string>(sp.get("cat") ?? "");
  ...
  useEffect(() => {
    (async () => {
      ...
      const p = await supabase
        .from("posts")
        .select("id,type,city,title,description,price_cents,price_unit,created_at,category_id,status")
        .eq("status", "active")
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-4xl p-4">
        ...
        <div className="mt-4 rounded-2xl border p-4 shadow-sm bg-white">
